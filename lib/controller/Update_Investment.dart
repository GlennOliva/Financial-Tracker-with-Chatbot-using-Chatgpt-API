...
class UpdateInvestment extends StatefulWidget {
  const UpdateInvestment({super.key, required this.investment});
...
Future<void> fetchUserData() async {
  try {
    final investmentId = widget.investment.id;
    final docSnapshot =
        await FirebaseFirestore.instance.collection('investment').doc(investmentId).get();

    if (docSnapshot.exists) {
      final investmentData = docSnapshot.data() as Map<String, dynamic>;
      final imageUrl = investmentData['image'] as String?;
...
Widget imgExist() {
 ...
 Future updateUser(String id) async {
  ...
  try {
    ...
    await FirebaseFirestore.instance.collection('investment').doc(id).update(updateData);
...
void initState() {
  ...
  isError = false;
  fetchUserData();
  namecontroller.text = widget.investment.name;
  amountcontroller.text = widget.investment.amount;
  timecontroller.text = widget.investment.time;
  descriptioncontroller.text = widget.investment.description;
  super.initState();
}
...
Widget build(BuildContext context) {
  return Scaffold(
    appBar: AppBar(
      title: const Text('UPDATE INVESTMENT'),
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    updateUser(widget.investment.id);
                  },
                  child: const Text('UPDATE INVESTMENT' , style: TextStyle(color: Colors.black),),

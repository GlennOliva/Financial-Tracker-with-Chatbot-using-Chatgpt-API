...
class UpdateIncome extends StatefulWidget {
  const UpdateIncome({super.key, required this.income});
...
Future<void> fetchUserData() async {
  try {
    final incomesId = widget.income.id;
    final docSnapshot =
        await FirebaseFirestore.instance.collection('income').doc(incomesId).get();

    if (docSnapshot.exists) {
      final incomesData = docSnapshot.data() as Map<String, dynamic>;
      final imageUrl = incomesData['image'] as String?;
...
Widget imgExist() {
 ...
 Future updateUser(String id) async {
  ...
  try {
    ...
    await FirebaseFirestore.instance.collection('income').doc(id).update(updateData);
...
void initState() {
  ...
  amountcontroller.text = widget.income.amount;
  timecontroller.text = widget.income.time;
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    updateUser(widget.income.id);
                  },
                  child: const Text('UPDATE INCOME' , style: TextStyle(color: Colors.black),),
